...
$compte++;
// generate qrcode
cmd("qrencode -s 3 -m 0 -o /dev/shm/qrtmp.png " . $array[$i][0]);
cmd("convert -bordercolor white -border 3 /dev/shm/qrtmp.png /dev/shm/code.png");
//cmd("rm /dev/shm/qrtmp.png");
...
cmd('convert -background white -fill black -font Arial-bold -pointsize 18 -size 420x36 -gravity west caption:"' . $array[$i][2] . '" /dev/shm/tmp1.png');
cmd('convert -background white -fill black -font Arial -pointsize 12 -size 420x33 -gravity west caption:"' . $array[$i][1] . ' \n ' . $array[$i][3] . '" /dev/shm/tmp2.png');
cmd('convert -append /dev/shm/tmp1.png /dev/shm/tmp2.png /dev/shm/tmp.png');
cmd('rm /dev/shm/tmp1.png');
cmd('rm /dev/shm/tmp2.png');
...
if ($compte > $lignes) {
	...
	if (($page % 2) == 0) {
		...
		cmd("convert +append /dev/shm/page" . $page-1 . ".png /dev/shm/page" . $page . ".png /dev/shm/Page" . sprintf('%02d', $Page) . ".png");
		cmd("rm /dev/shm/page*");
		$page=0;
		$Page++;
		}
...
//cmd("img2pdf --output /dev/shm/sortie.pdf --pagesize A4 --border 0.5cm:0.5cm /dev/shm/Page*");
cmd("img2pdf --output /dev/shm/sortie.pdf --pagesize A4 /dev/shm/Page*");

echo "<br><br><b>Votre fichier est prêt : </b>";
echo "<a href='etiqd.php'>Fichier étiquettes</a>";
cmd("rm /dev/shm/*.png");
?>
